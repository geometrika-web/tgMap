...
select.getFeatures().on(['add', 'remove'], function (e) {
    if (currZoom > 11.5) {
        if (e.type == 'add') {
            ...
            document.getElementById('landmark').style.display = 'inline-block';
            document.getElementById('image').style.display = 'inline-block';
            document.getElementById('landmark-about').style.display =
                'inline-block';
            document.getElementById('nodelist').style.display = 'none';
            document.getElementById('landmark').innerHTML = feature.get('name');
...
function getSourceJSON(clickedID, selected) {
    ...
    map.getLayers().forEach(function (layer) {
        ...
        if (name) {
            ...
            if (name == clickedID) {
                let clickedIDtext = document.getElementById(clickedID + '-text')
                    .style.display;
...
function zoomToAssoc(id) {
    ...
    if (id == 'assoc1') {
        view.animate({
            center: ol.proj.fromLonLat([17.49951, 43.614876]),
            duration: 1000,
            zoom: 16.5
        });
    }
    if (id == 'assoc2') {
        view.animate({
            center: ol.proj.fromLonLat([17.49951, 43.624876]),
            duration: 1000,
            zoom: 16
        });
    }
